...
export default class ChatWebSocket {
  ...
  private init() {
    ...
    this.webSocket.addEventListener("message", (event) => {
      ...
      let callback: ((body: any) => void) | undefined;
  ...
  private decodeArrayBuffer(arrayBuffer: ArrayBuffer | Array<any>): string | null {
    if (arrayBuffer) {
      return decodeURIComponent(window.escape(String.fromCharCode.apply(null, new Uint8Array(arrayBuffer) as any)));
    }
    return null;
  }
  /**
  ...
  private convertToObject(arrayBuffer: ArrayBuffer): resObj {
    ...
    if (res.op === wsConstant.WS_OP_HEARTBEAT_REPLY && res.headerLen) {
      // 心跳回应，返回的内容为32位整型
      ...
      // buffer内容
      if (res.ver === wsConstant.WS_BODY_PROTOCOL_VERSION_DEFLATE && res.headerLen) {
        ...
        const bodyLength = dataView.byteLength - Number(res.headerLen);
        const array: any[] = [];
        for (let i = 0; i < bodyLength; i++) {
          array.push(dataView.getUint8(Number(res.headerLen) + i));
        }
        res.body = [JSON.parse(this.decodeArrayBuffer(array) as any)];
      }
    }
  ...
  private getBody(arrayBuffer: ArrayBuffer, body: Array<any>) {
    ...
    body.push(JSON.parse(this.decodeArrayBuffer(arrayBuffer.slice(headerLen, packetLen)) as any));
